...
const defaultProps = {
  dataBox: () => ({ variant: 'facebook', friends: '-', feeds: '-', module:'-', icon:'-' }),
};
...
class Widget03 extends Component {
  render() {
    ...
    if (!variant || ['facebook', 'twitter', 'linkedin', 'google-plus', 'info', 'success', 'danger', 'warning'].indexOf(variant) < 0) {
      return (null);
    }
    ...
    return (
      <div className={classes}>
        <div className={classCardHeader}>
          <i className={icon}></i>
          <div className="text-value ml-2">{module}</div>
          {children}
        </div>
